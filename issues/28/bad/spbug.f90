...
module spbug
 ...
 contains
  ...
  procedure, private :: swigf_assign_Ct
  generic :: assignment(=) => swigf_assign_Ct
 end type
 ...
 interface
  ...
  function swigc_spcopy_Ct(farg1) &
     bind(C, name="swigc_spcopy_Ct") &
     result(fresult)
   use, intrinsic :: ISO_C_BINDING
   type(C_PTR) :: fresult
   type(C_PTR), value :: farg1
...
contains
  ...
  subroutine swigf_new_Ct(self, x)
   use, intrinsic :: ISO_C_BINDING
   class(Ct) :: self
   class(SWIGTYPE_p_std__shared_ptrT_nspace__A_t) :: x
  ...
  subroutine swigf_assign_Ct(self, other)
   use, intrinsic :: ISO_C_BINDING
   class(Ct), intent(inout) :: self
   type(Ct), intent(in) :: other
   call self%release()
   self%swigptr = swigc_spcopy_Ct(other%swigptr)
  end subroutine
end module spbug
