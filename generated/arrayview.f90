! This file was automatically generated by SWIG (http://www.swig.org).
! Version 4.0.0
!
! Do not make changes to this file unless you know what you are doing--modify
! the SWIG interface file instead.
module arrayview
 use, intrinsic :: ISO_C_BINDING
 implicit none
 private

 ! PUBLIC METHODS AND TYPES

type, bind(C) :: SwigArrayWrapper
  type(C_PTR), public :: data = C_NULL_PTR
  integer(C_SIZE_T), public :: size = 0
end type

 public :: reverse
 interface reverse
  module procedure reverse__SWIG_1, reverse__SWIG_2, reverse__SWIG_3
 end interface
 public :: find_sorted
 interface find_sorted
  module procedure find_sorted__SWIG_1, find_sorted__SWIG_2, find_sorted__SWIG_3
 end interface
 public :: get_view
 interface get_view
  module procedure get_view__SWIG_1, get_view__SWIG_2, get_view__SWIG_3
 end interface
 public :: sort
 interface sort
  module procedure sort__SWIG_1, sort__SWIG_2, sort__SWIG_3
 end interface
 public :: print_array
 interface print_array
  module procedure print_array__SWIG_1, print_array__SWIG_2, print_array__SWIG_3
 end interface

 ! WRAPPER DECLARATIONS
 interface
subroutine swigc_sort__SWIG_1(farg1) &
bind(C, name="swigc_sort__SWIG_1")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

subroutine swigc_reverse__SWIG_1(farg1) &
bind(C, name="swigc_reverse__SWIG_1")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

function swigc_find_sorted__SWIG_1(farg1, farg2) &
bind(C, name="swigc_find_sorted__SWIG_1") &
result(fresult)
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
integer(C_INT), intent(in) :: farg2
integer(C_INT) :: fresult
end function

function swigc_get_view__SWIG_1(farg1) &
bind(C, name="swigc_get_view__SWIG_1") &
result(fresult)
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
type(SwigArrayWrapper) :: fresult
end function

subroutine swigc_print_array__SWIG_1(farg1) &
bind(C, name="swigc_print_array__SWIG_1")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

subroutine swigc_sort__SWIG_2(farg1) &
bind(C, name="swigc_sort__SWIG_2")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

subroutine swigc_reverse__SWIG_2(farg1) &
bind(C, name="swigc_reverse__SWIG_2")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

function swigc_find_sorted__SWIG_2(farg1, farg2) &
bind(C, name="swigc_find_sorted__SWIG_2") &
result(fresult)
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
real(C_FLOAT), intent(in) :: farg2
integer(C_INT) :: fresult
end function

function swigc_get_view__SWIG_2(farg1) &
bind(C, name="swigc_get_view__SWIG_2") &
result(fresult)
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
type(SwigArrayWrapper) :: fresult
end function

subroutine swigc_print_array__SWIG_2(farg1) &
bind(C, name="swigc_print_array__SWIG_2")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

subroutine swigc_sort__SWIG_3(farg1) &
bind(C, name="swigc_sort__SWIG_3")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

subroutine swigc_reverse__SWIG_3(farg1) &
bind(C, name="swigc_reverse__SWIG_3")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

function swigc_find_sorted__SWIG_3(farg1, farg2) &
bind(C, name="swigc_find_sorted__SWIG_3") &
result(fresult)
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
real(C_DOUBLE), intent(in) :: farg2
integer(C_INT) :: fresult
end function

function swigc_get_view__SWIG_3(farg1) &
bind(C, name="swigc_get_view__SWIG_3") &
result(fresult)
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
type(SwigArrayWrapper) :: fresult
end function

subroutine swigc_print_array__SWIG_3(farg1) &
bind(C, name="swigc_print_array__SWIG_3")
use, intrinsic :: ISO_C_BINDING
import :: SwigArrayWrapper
type(SwigArrayWrapper) :: farg1
end subroutine

 end interface


contains
 ! FORTRAN PROXY CODE
subroutine sort__SWIG_1(view)
use, intrinsic :: ISO_C_BINDING
integer(C_INT), dimension(:), target :: view
integer(C_INT), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_sort__SWIG_1(farg1)
end subroutine

subroutine reverse__SWIG_1(view)
use, intrinsic :: ISO_C_BINDING
integer(C_INT), dimension(:), target :: view
integer(C_INT), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_reverse__SWIG_1(farg1)
end subroutine

function find_sorted__SWIG_1(view, val) &
result(swig_result)
use, intrinsic :: ISO_C_BINDING
integer(C_INT) :: swig_result
integer(C_INT), dimension(:), target :: view
integer(C_INT), pointer :: farg1_view
integer(C_INT), intent(in) :: val
integer(C_INT) :: fresult 
type(SwigArrayWrapper) :: farg1 
integer(C_INT) :: farg2 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
farg2 = val
fresult = swigc_find_sorted__SWIG_1(farg1, farg2)
swig_result = fresult
end function

function get_view__SWIG_1(view) &
result(swig_result)
use, intrinsic :: ISO_C_BINDING
integer(C_INT), dimension(:), pointer :: swig_result
integer(C_INT), dimension(:), target :: view
integer(C_INT), pointer :: farg1_view
type(SwigArrayWrapper) :: fresult 
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
fresult = swigc_get_view__SWIG_1(farg1)
call c_f_pointer(fresult%data, swig_result, [fresult%size])
end function

subroutine print_array__SWIG_1(view)
use, intrinsic :: ISO_C_BINDING
integer(C_INT), dimension(:), target :: view
integer(C_INT), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_print_array__SWIG_1(farg1)
end subroutine

subroutine sort__SWIG_2(view)
use, intrinsic :: ISO_C_BINDING
real(C_FLOAT), dimension(:), target :: view
real(C_FLOAT), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_sort__SWIG_2(farg1)
end subroutine

subroutine reverse__SWIG_2(view)
use, intrinsic :: ISO_C_BINDING
real(C_FLOAT), dimension(:), target :: view
real(C_FLOAT), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_reverse__SWIG_2(farg1)
end subroutine

function find_sorted__SWIG_2(view, val) &
result(swig_result)
use, intrinsic :: ISO_C_BINDING
integer(C_INT) :: swig_result
real(C_FLOAT), dimension(:), target :: view
real(C_FLOAT), pointer :: farg1_view
real(C_FLOAT), intent(in) :: val
integer(C_INT) :: fresult 
type(SwigArrayWrapper) :: farg1 
real(C_FLOAT) :: farg2 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
farg2 = val
fresult = swigc_find_sorted__SWIG_2(farg1, farg2)
swig_result = fresult
end function

function get_view__SWIG_2(view) &
result(swig_result)
use, intrinsic :: ISO_C_BINDING
real(C_FLOAT), dimension(:), pointer :: swig_result
real(C_FLOAT), dimension(:), target :: view
real(C_FLOAT), pointer :: farg1_view
type(SwigArrayWrapper) :: fresult 
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
fresult = swigc_get_view__SWIG_2(farg1)
call c_f_pointer(fresult%data, swig_result, [fresult%size])
end function

subroutine print_array__SWIG_2(view)
use, intrinsic :: ISO_C_BINDING
real(C_FLOAT), dimension(:), target :: view
real(C_FLOAT), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_print_array__SWIG_2(farg1)
end subroutine

subroutine sort__SWIG_3(view)
use, intrinsic :: ISO_C_BINDING
real(C_DOUBLE), dimension(:), target :: view
real(C_DOUBLE), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_sort__SWIG_3(farg1)
end subroutine

subroutine reverse__SWIG_3(view)
use, intrinsic :: ISO_C_BINDING
real(C_DOUBLE), dimension(:), target :: view
real(C_DOUBLE), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_reverse__SWIG_3(farg1)
end subroutine

function find_sorted__SWIG_3(view, val) &
result(swig_result)
use, intrinsic :: ISO_C_BINDING
integer(C_INT) :: swig_result
real(C_DOUBLE), dimension(:), target :: view
real(C_DOUBLE), pointer :: farg1_view
real(C_DOUBLE), intent(in) :: val
integer(C_INT) :: fresult 
type(SwigArrayWrapper) :: farg1 
real(C_DOUBLE) :: farg2 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
farg2 = val
fresult = swigc_find_sorted__SWIG_3(farg1, farg2)
swig_result = fresult
end function

function get_view__SWIG_3(view) &
result(swig_result)
use, intrinsic :: ISO_C_BINDING
real(C_DOUBLE), dimension(:), pointer :: swig_result
real(C_DOUBLE), dimension(:), target :: view
real(C_DOUBLE), pointer :: farg1_view
type(SwigArrayWrapper) :: fresult 
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
fresult = swigc_get_view__SWIG_3(farg1)
call c_f_pointer(fresult%data, swig_result, [fresult%size])
end function

subroutine print_array__SWIG_3(view)
use, intrinsic :: ISO_C_BINDING
real(C_DOUBLE), dimension(:), target :: view
real(C_DOUBLE), pointer :: farg1_view
type(SwigArrayWrapper) :: farg1 

farg1_view => view(1)
farg1%data = c_loc(farg1_view)
farg1%size = size(view)
call swigc_print_array__SWIG_3(farg1)
end subroutine


end module
